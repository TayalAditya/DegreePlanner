import { normalizeBatchYear } from "@/lib/courseCategory";

export type MinorRequirementGroup = {
  id: string;
  title: string;
  requiredCount: number;
  courseCodes: string[];
  /**
   * Courses in one set are alternatives for this minor requirement: completing
   * or planning more than one of them still fulfils only one slot. This is
   * deliberately scoped to the minor, rather than a global course equivalence.
   */
  alternativeCourseCodeSets?: string[][];
  countsTowardMinor: boolean;
  note?: string;
};

export type MinorDefinition = {
  code: string;
  name: string;
  totalCreditsRequired?: number;
  /** If set, this can also be pursued as a Specialization requiring this many credits (≥ minor). */
  specializationCreditsRequired?: number;
  /** If set, only students from these batch years see this minor. */
  eligibleBatches?: number[];
  groups: MinorRequirementGroup[];
};

export const MINORS: MinorDefinition[] = [
  {
    code: "MGMT",
    name: "Minor in Management (B23)",
    totalCreditsRequired: 12,
    eligibleBatches: [2023],
    groups: [
      {
        id: "prereq-ic252",
        title: "Prerequisite: IC-252",
        requiredCount: 1,
        courseCodes: ["IC-252"],
        countsTowardMinor: false,
      },
      {
        id: "prereq-comm",
        title: "Prerequisite: Communicative Competence",
        requiredCount: 1,
        courseCodes: [
          "HS-105",
          "HS-109",
          "HS-305",
          "HS-357",
          "MB-304",
          "MB-205",
          "MB-305",
        ],
        countsTowardMinor: false,
        note: "Any 1 from the Communicative Competence basket.",
      },
      {
        id: "core-econ",
        title: "Core: Economics (pick 1)",
        requiredCount: 1,
        courseCodes: ["HS-202", "HS-481"],
        countsTowardMinor: true,
      },
      {
        id: "core-mgmt",
        title: "Core: Management (pick 1)",
        requiredCount: 1,
        courseCodes: ["HS-304", "MB-201"],
        countsTowardMinor: true,
      },
      {
        id: "electives",
        title: "Electives",
        requiredCount: 2,
        courseCodes: [
          "HS-307",
          "MB-203",
          "HS-510",
          "HS-504",
          "MB-307",
          "MB-524",
          "MB-582",
          "MB-526",
          "MB-527",
          "MB-528",
          "MB-562",
          "MB-570",
        ],
        alternativeCourseCodeSets: [["HS-510", "HS-504"]],
        countsTowardMinor: true,
        note: "Any 2 courses (6 credits) from the electives basket. HS-510 and HS-504 are alternatives, so only one counts.",
      },
    ],
  },
  {
    code: "MGMT_B24",
    name: "Minor in Management (B24 onwards)",
    totalCreditsRequired: 12,
    eligibleBatches: [2024, 2025, 2026],
    groups: [
      {
        id: "core",
        title: "Compulsory Core Courses (both required)",
        requiredCount: 2,
        courseCodes: ["MB-311", "MB-312"],
        countsTowardMinor: true,
        note: "Both core courses must be completed before taking electives. Sem 5-6 recommended.",
      },
      {
        id: "electives",
        title: "Electives (pick 2)",
        requiredCount: 2,
        courseCodes: [
          "MB-410",
          "MB-411",
          "MB-412",
          "MB-413",
          "MB-414",
          "MB-415",
          "MB-416",
        ],
        countsTowardMinor: true,
        note: "Any 2 elective courses (6 credits). Sem 7-8 recommended, after core completion.",
      },
    ],
  },
  {
    code: "CSE",
    name: "Minor in Computer Science Engineering",
    totalCreditsRequired: 9,
    groups: [
      {
        id: "prereq-ic253",
        title: "Prerequisite: IC-253 (doesn’t count)",
        requiredCount: 1,
        courseCodes: ["IC-253"],
        countsTowardMinor: false,
        note: "IC-253 is a prerequisite for CS-212, but does not count towards this minor.",
      },
      {
        id: "compulsory",
        title: "Compulsory",
        requiredCount: 1,
        courseCodes: ["CS-212"],
        countsTowardMinor: true,
      },
      {
        id: "electives",
        title: "Electives",
        requiredCount: 2,
        courseCodes: [
          "CS-313",
          "CS-302",
          "CS-305",
          "CS-214",
          "CS-304",
          "CS-309",
          "CS-208",
          "CS-312",
        ],
        countsTowardMinor: true,
      },
    ],
  },
  {
    code: "IS",
    name: "Minor in Intelligent Systems",
    totalCreditsRequired: 9,
    groups: [
      {
        id: "basket",
        title: "Course Basket",
        requiredCount: 3,
        courseCodes: [
          "EE-511",
          "CS-673",
          "CS-671",
          "CS-672",
          "DS-413",
          "DS-411",
          "CS-683",
          "CS-685",
          "CS-305",
          "DS-412",
          "EE-608",
          "BY-606",
          "CS-630",
          "CS-660",
          "CS-609",
        ],
        countsTowardMinor: true,
        note: "No compulsory courses; any counted course should be non P/F and non Audit.",
      },
    ],
  },
  {
    code: "POWER",
    name: "Minor in Power Engineering",
    totalCreditsRequired: 9,
    groups: [
      {
        id: "compulsory",
        title: "Compulsory",
        requiredCount: 2,
        courseCodes: ["EE-201", "EE-309"],
        countsTowardMinor: true,
      },
      {
        id: "electives",
        title: "Electives",
        requiredCount: 1,
        courseCodes: ["CS-403", "CS-304", "CS-214", "CS-309", "CS-671"],
        countsTowardMinor: true,
      },
    ],
  },
  {
    code: "THERMO",
    name: "Minor in Thermo-Fluid Systems",
    totalCreditsRequired: 9,
    groups: [
      {
        id: "basket",
        title: "Course Basket",
        requiredCount: 3,
        courseCodes: [
          "ME-210",
          "ME-303",
          "ME-307",
          "ME-356",
          "ME-451",
          "ME-614",
          "ME-615",
        ],
        countsTowardMinor: true,
      },
    ],
  },
  {
    code: "ELECTRONICS",
    name: "Minor in Electronics Engineering",
    totalCreditsRequired: 9,
    groups: [
      {
        id: "compulsory",
        title: "Compulsory",
        requiredCount: 2,
        courseCodes: ["EE-203", "EE-210"],
        countsTowardMinor: true,
      },
      {
        id: "electives",
        title: "Electives",
        requiredCount: 1,
        courseCodes: ["EE-305", "EE-314", "EE-311", "EE-211", "EE-312P"],
        countsTowardMinor: true,
        note: "EE-305 and EE-314 are equivalent — only one counts.",
      },
    ],
  },
  {
    code: "COMM",
    name: "Minor in Communication Engineering",
    totalCreditsRequired: 9,
    groups: [
      {
        id: "compulsory",
        title: "Compulsory",
        requiredCount: 1,
        courseCodes: ["EE-304"],
        countsTowardMinor: true,
      },
      {
        id: "electives",
        title: "Electives",
        requiredCount: 2,
        courseCodes: ["EE-503", "EE-314", "EE-530"],
        countsTowardMinor: true,
      },
    ],
  },
  {
    code: "MECH_DES",
    name: "Minor in Mechanical Design",
    totalCreditsRequired: 9,
    groups: [
      {
        id: "basket",
        title: "Course Basket",
        requiredCount: 3,
        courseCodes: ["ME-205", "ME-309", "ME-206", "ME-352", "ME-305", "ME-602"],
        countsTowardMinor: true,
      },
    ],
  },
  {
    code: "MEAS_INSTR",
    name: "Minor in Measurement and Instrumentation",
    totalCreditsRequired: 9,
    groups: [
      {
        id: "compulsory",
        title: "Compulsory",
        requiredCount: 1,
        courseCodes: ["EE-313"],
        countsTowardMinor: true,
      },
      {
        id: "electives",
        title: "Electives",
        requiredCount: 2,
        courseCodes: ["EE-301", "EE-314", "EE-62010", "EE-62024"],
        countsTowardMinor: true,
        note: "Advanced DSP appears as EE 620 10 / EE 620 24 in the curriculum.",
      },
    ],
  },
  {
    code: "MATERIALS",
    name: "Minor in Device / Structural Materials",
    totalCreditsRequired: 9,
    groups: [
      {
        id: "basket",
        title: "Course Basket",
        requiredCount: 3,
        courseCodes: ["ME-353", "ME-609", "ME-607", "ME-619"],
        countsTowardMinor: true,
      },
    ],
  },
  {
    code: "CONTROL",
    name: "Minor in Control Engineering",
    totalCreditsRequired: 9,
    groups: [
      {
        id: "compulsory",
        title: "Compulsory",
        requiredCount: 2,
        courseCodes: ["EE-301", "EE-301P"],
        countsTowardMinor: true,
      },
      {
        id: "electives",
        title: "Electives",
        requiredCount: 1,
        courseCodes: ["EE-509", "EE-514"],
        countsTowardMinor: true,
      },
    ],
  },
  {
    code: "GERMAN",
    name: "Minor in German Language",
    totalCreditsRequired: 9,
    groups: [
      {
        id: "basket",
        title: "Course Basket",
        requiredCount: 3,
        courseCodes: ["HS-352", "HS-362", "HS-363", "HS-372", "HS-373", "HS-539"],
        countsTowardMinor: true,
      },
    ],
  },
  {
    code: "APPLIED_PHYS",
    name: "Minor in Applied Physics",
    totalCreditsRequired: 12,
    groups: [
      {
        id: "basket",
        title: "Course Basket",
        requiredCount: 4,
        courseCodes: [
          "EP-502",
          "PH-302",
          "PH-522",
          "PH-301",
          "PH-513",
          "PH-501",
          "PH-523",
          "PH-502",
          "PH-503",
          "PH-504",
          "PH-505",
          "PH-506",
          "PH-507",
          "PH-508",
          "PH-511",
          "PH-528",
          "PH-601",
          "PH-603",
          "PH-604",
          "PH-605",
          "PH-606",
          "PH-607",
          "PH-608",
          "PH-609",
          "PH-612",
          "PH-701",
          "PH-706",
        ],
        countsTowardMinor: true,
      },
    ],
  },
  {
    code: "QUANTUM",
    name: "Quantum Technologies",
    totalCreditsRequired: 12,
    specializationCreditsRequired: 18,
    groups: [
      {
        id: "foundation-survey",
        title: "QT-301 — Survey of Quantum Technologies (Mandatory)",
        requiredCount: 1,
        courseCodes: ["QT-301"],
        countsTowardMinor: true,
        note: "QT-301 is always mandatory for both minor and specialization.",
      },
      {
        id: "foundation-theory",
        title: "Foundations of Quantum Technologies (pick 1)",
        requiredCount: 1,
        courseCodes: ["QT-302", "PH-513", "EP-301"],
        countsTowardMinor: true,
        note: "QT-302 has ~100% overlap with PH-513 and EP-301 — only one counts.",
      },
      {
        id: "lab",
        title: "Lab (pick 1)",
        requiredCount: 1,
        courseCodes: ["QT-303P", "QT-304P", "QS-501P"],
        countsTowardMinor: true,
        note: "At least one lab course is mandatory. QT-304P and QS-501P are equivalent.",
      },
      {
        id: "advanced",
        title: "Advanced Topics (pick 1)",
        requiredCount: 1,
        courseCodes: ["QT-405", "CS-520", "QT-406", "QT-407", "QT-408", "PH-523", "PH-501"],
        countsTowardMinor: true,
        note: "QT-405/CS-520 are equivalent; QT-408/PH-523/PH-501 are equivalent — only one from each pair counts.",
      },
      {
        id: "optional",
        title: "Additional (optional)",
        requiredCount: 1,
        courseCodes: ["QT-509", "QT-510", "PH-601", "PH-603", "PH-604", "PH-605", "QT-511", "PH-550", "CS-521"],
        countsTowardMinor: true,
        note: "Optional for minor (12 cr already met by above). Pick 2 courses (6 cr) to qualify for Specialization (18+ cr total).",
      },
    ],
  },
  {
    code: "ROBOTICS",
    name: "Minor in Robotics (B23–B24)",
    totalCreditsRequired: 12,
    eligibleBatches: [2023, 2024],
    groups: [
      {
        id: "compulsory-1",
        title: "Compulsory: Robot Kinematics/Dynamics/Control (pick 1)",
        requiredCount: 1,
        courseCodes: ["AR-501", "ME-452"],
        countsTowardMinor: true,
      },
      {
        id: "compulsory-2",
        title: "Compulsory: Mechatronics (pick 1)",
        requiredCount: 1,
        courseCodes: ["AR-503", "AR-520"],
        countsTowardMinor: true,
        note: "AR-503 or AR-520 (equivalent courses — complete either one).",
      },
      {
        id: "electives",
        title: "Electives",
        requiredCount: 2,
        courseCodes: [
          "AR-502",
          "AR-504",
          "AR-505",
          "AR-506",
          "AR-507",
          "AR-508",
          "AR-509",
          "AR-510",
          "AR-511",
          "AR-512",
          "AR-513",
          "AR-514",
          "AR-515",
          "AR-519",
          "EE-555",
        ],
        countsTowardMinor: true,
        note: "Any 2 out of the electives list (as per curriculum).",
      },
    ],
  },
  {
    code: "ROBOTICS_B25",
    name: "Minor in Robotics (B25 onwards)",
    totalCreditsRequired: 12,
    eligibleBatches: [2025, 2026, 2027, 2028],
    groups: [
      {
        id: "core",
        title: "Compulsory Core Courses (both required)",
        requiredCount: 2,
        courseCodes: ["AR-523", "AR-520"],
        countsTowardMinor: true,
        note: "AR-523 Robot Manipulators + AR-520 Design of Mechatronic Systems (6 credits total).",
      },
      {
        id: "electives",
        title: "Electives (pick 2)",
        requiredCount: 2,
        courseCodes: [
          "AR-502",
          "AR-504",
          "AR-505",
          "AR-506",
          "AR-507",
          "AR-508",
          "AR-509",
          "AR-510",
          "AR-511",
          "AR-512",
          "AR-513",
          "AR-514",
          "AR-515",
          "AR-521",
          "AR-522",
          "AR-524",
          "AR-525",
          "AR-526",
          "AR-527",
          "EE-555",
        ],
        countsTowardMinor: true,
        note: "Any 2 electives from the list (6 credits). New electives may be added by the Centre from time to time.",
      },
    ],
  },
  {
    code: "VLSI_DESIGN",
    name: "Minor in VLSI Design (B25 onwards)",
    totalCreditsRequired: 9,
    groups: [
      {
        id: "all",
        title: "Compulsory (all 3 required)",
        requiredCount: 3,
        courseCodes: ["VL-404", "EE-524", "EE-519P"],
        countsTowardMinor: true,
        note: "VL-404 CMOS Analog IC Design (4 cr) + EE-524 Digital MOS LSI Circuits (3 cr) + EE-519P Digital IC Design Practicum (2 cr). All three courses are compulsory.",
      },
    ],
  },
  {
    code: "VLSI_TECH",
    name: "Minor in VLSI Technology (B25 onwards)",
    totalCreditsRequired: 10,
    groups: [
      {
        id: "all",
        title: "Compulsory (all 3 required)",
        requiredCount: 3,
        courseCodes: ["VL-311", "VL-312", "EE-615"],
        countsTowardMinor: true,
        note: "VL-311 CMOS Processing and Practicum (4 cr) + VL-312 Electronic System Packaging (3 cr) + EE-615 Nanoelectronics and Nano-Microfabrication (3 cr). All three courses are compulsory.",
      },
    ],
  },
];

function normalizeMinorCourseCode(code: string): string {
  return code.toUpperCase().replace(/[^A-Z0-9]/g, "");
}

/**
 * Minors visible to a given batch.
 *
 * Use this everywhere a minor list is rendered. The Programs page and the course
 * registration planner previously filtered independently — the registration page
 * applied `eligibleBatches` while the Programs page applied nothing — so a minor
 * could appear in one and not the other. Routing both through this helper is what
 * keeps them in step.
 *
 * A minor with no `eligibleBatches` is open to every batch; where a minor is only
 * offered from a given batch onward, that belongs in its `name` (e.g. "(B25
 * onwards)") so students still see it exists rather than silently losing it.
 *
 * `batchYear` may be null when a profile has no batch and no parseable roll
 * number; in that case show the unrestricted minors rather than an empty list.
 */
export function getEligibleMinors(batchYear?: number | string | null): MinorDefinition[] {
  const batch = normalizeBatchYear(batchYear);
  return MINORS.filter(
    (minor) => !minor.eligibleBatches || (batch != null && minor.eligibleBatches.includes(batch))
  );
}

/** Returns a stable key for counting a course inside one minor requirement. */
export function getMinorRequirementCountingKey(
  group: MinorRequirementGroup,
  courseCode: string
): string {
  const normalizedCode = normalizeMinorCourseCode(courseCode);
  const alternativeIndex = group.alternativeCourseCodeSets?.findIndex((set) =>
    set.some((candidate) => normalizeMinorCourseCode(candidate) === normalizedCode)
  );

  return alternativeIndex === undefined || alternativeIndex < 0
    ? `course:${normalizedCode}`
    : `alternative:${alternativeIndex}`;
}

/** Whether two different courses occupy the same OR slot in a minor requirement. */
export function areMinorRequirementAlternatives(
  group: MinorRequirementGroup,
  firstCourseCode: string,
  secondCourseCode: string
): boolean {
  const first = normalizeMinorCourseCode(firstCourseCode);
  const second = normalizeMinorCourseCode(secondCourseCode);
  if (!first || !second || first === second) return false;

  return Boolean(
    group.alternativeCourseCodeSets?.some((set) => {
      const normalizedSet = new Set(set.map(normalizeMinorCourseCode));
      return normalizedSet.has(first) && normalizedSet.has(second);
    })
  );
}
